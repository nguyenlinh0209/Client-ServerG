...
public class Client_Model extends Thread{
	...
	public void Send(String data) throws IOException {
		DataInputStream dataInputStream = new DataInputStream(socket.getInputStream());// lấy luồng dl đi vào từ socket (đocj dl)
		DataOutputStream dataOutputStream = new DataOutputStream(socket.getOutputStream());// lấy luồng dl đi ra từ client 
		dataOutputStream.writeUTF(data); // ghi dl
		...
		while(true) {
			try {
				...
				if (key[0].equals("NewGame")) {
					...
					block = true;
				}else if(key[0].equals("GameOver")) {
					gameOver = true;
					if (key.length > 1) {
						lives = Integer.parseInt(key[1]);
					}
				}
