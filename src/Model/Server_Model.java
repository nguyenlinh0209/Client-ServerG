		...
		while(true) {
			try {
				...
				for (WorkThread w : arrayListWorkThread) {
					...
					w.setArrayListWorkThread(arrayListWorkThread);
				}
...
class WorkThread extends Thread{
	...
	ArrayList<WorkThread> arrayListWorkThread;
	...
	public void setArrayListWorkThread(ArrayList<WorkThread> arrayListWorkThread) {
		this.arrayListWorkThread = arrayListWorkThread;
	}
			...
			while(true) {
				...
				if (key[0].equals("Click")) {
					...
					if (check(st, data.getText())) {
						...
						String loserName = key[3];
						for (WorkThread w : arrayListWorkThread) {
							if (w.userName.equals(loserName)) {
								w.lives--; // giảm mạng 
								if (w.lives <= 0) {
									Send("Block", w.socket);
								} else {
									Send("GameOver " + w.lives, w.socket);
								}
							} else {
								Send("GameOver", w.socket);
							}
							Send("BXHserver", w.socket);
							SendObj(dataSQL_Model.getListUser(), w.socket);
						}
					}
				...
				} else if(key[0].equals("PlayAgain")) {
					data.setText("3 1 1 1");
					for (Socket s : arrayListSocket) {
						Send("NewGame " + data.getText(), s);
					}
				}
